...
public class BasicTurret extends Weapon {
	...
	public BasicTurret(Tank t, double atot, double dtot){
		...
		setWeaponShape(new Polygon(x, y, 9));
		//Rotate it to its initial location
		setWeaponShape(Transform.transform(getWeaponShape(), t.getCenter().getX() + dtot * Math.cos(Math.toRadians(t.getTheta() + atot)), t.getCenter().getY() + dtot * Math.sin(Math.toRadians(t.getTheta() + atot)), Math.toRadians(-90), t.getCenter().getX() + dtot * Math.cos(Math.toRadians(t.getTheta() + atot)), t.getCenter().getY() + dtot * Math.sin(Math.toRadians(t.getTheta() + atot))));
	}
