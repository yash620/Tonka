...
public class Block implements Drawable, Collidable {
	...
	public double getBoundsArea(){
		return blockShape.getBounds().getHeight() * blockShape.getBounds().getWidth();
	}
	@Override
	...
	public boolean isSplit(){
		return !blockShape.isSingular();
	}
	public ArrayList<Block> splitBlock(){
		/*If the area is seperated into several bits,
		 * then make two different blocks representing the bits
		 * 
		 *Store all of the path iterator points inside a double array
		 * in the form (type, x, y)
		 */
		...
		/* Creates new Polygons for each of the distinct paths
		 * It searches for the SEG_MOVETO, which is the start of a
		 * new path
		 */
		...
		for (double[] p : points){
			if (p[0] == PathIterator.SEG_CLOSE){
				if (!xCoords.isEmpty()){
					...
					Block addBlock = new Block((Shape)new Polygon(xArr, yArr, xCoords.size()));
